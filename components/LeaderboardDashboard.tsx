...
export default function LeaderboardDashboard({
  ...
  const activeGroupName =
    groups.find((group) => group.id === activeGroupId)?.name ?? groups[0]?.name ?? null;
    ...
    <div className="w-full">
      <div className="mt-2 mb-2 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          {groups.length > 0 ? (
            <label className="hidden items-center gap-2 rounded-full border border-[#dbc7a4] bg-white/80 px-3 py-1.5 text-xs font-semibold text-[#7c5b1f] shadow-[0_10px_20px_rgba(15,23,42,0.06)] sm:flex">
          ...
          <div className="hidden rounded-full border border-[#dbc7a4] bg-white/80 px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-[#7c5b1f] shadow-[0_10px_20px_rgba(15,23,42,0.06)] sm:block">
            {activeGroupName ? `${activeGroupName} view` : 'Private league view'}
          </div>
        </div>
      </div>
